...
func RestartPollers(db *sql.DB, hub *wsHub.Hub, uStore *userStore.Store, auth *spotify.Authenticator) {
...
func PollPlayerForRemoval(client *spotify.Client, roomCode string, hub *wsHub.Hub,
	...
	for retryCount < 5 {
		...
		CheckAddRandomSong(client, roomCode, hub)
...
// CheckAddRandomSong is used to see if a new song needs to be added to the
// playlist. If there is only one song currently in the playlist (queue), it
// will then add a random song to the playlist.
func CheckAddRandomSong(client *spotify.Client, roomCode string, hub *wsHub.Hub) {
	playlistID := GetPlaylistIdByName(client, "GroupQueue")
	...
	if len(tracks.Tracks) <= 1 {
		if err := addRandomSong(client, playlistID, tracks.Tracks, roomCode, hub); err != nil {
			log.Println(err)
		}
	}
}
...
func addRandomSong(client *spotify.Client, playlistID spotify.ID, tracks []spotify.PlaylistTrack,
	roomCode string, hub *wsHub.Hub) error {
	if topPlaylistId == "" {
